      ...
      parsedObjects.forEach((objData: fabric.Object) => {
        ...
        fabric.util.enlivenObjects([objData]).then((enlivenedObjects) => {
          enlivenedObjects.forEach((enlivenedObj) => {
            ...
            (enlivenedObj as fabric.FabricObject).set({
              left: (enlivenedObj as fabric.FabricObject).left || 0 + 20,
              top: (enlivenedObj as fabric.FabricObject).top || 0 + 20,
              objectId: uuidv4(),
              fill: "#aabbcc",
            } as CustomFabricObject<any>);

            canvas.add(enlivenedObj as fabric.FabricObject);
            syncShapeInStorage(enlivenedObj as fabric.FabricObject);
          });
          canvas.renderAll();
        });
      });
